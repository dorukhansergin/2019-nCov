...
from bokeh.models import TextInput, Div, Slider, Panel, Tabs
...
spread_factors_control_panel_ui_params = {
    ...
    "TtSO": {"widget": Slider, "kwargs": {"start": 1, "end": 14, "value": 5, "step": 1, "title": "Time to Symptom Onset(years)"}, "type": int},
...
class QuarantineTwo(BaseModel):
    def dynamic_control_panel(self):
        ...
        return row(Tabs(tabs=[Panel(child=pop_control_panel, title="Initial Variables of Stocks"), Panel(child=spread_factors_panel, title="Parameters")]), id="dynamic-control-panel")
    ...
    def parse_control_panel_values_to_engine_init(self, control_panel):
        ...
        population_initials_dict = {key: int(value) for key, value in zip(["S", "IA", "IPs", "ISHome", "ISHosp"], [child.value for child in pop_control_panel.children])}
        R0, SAR, InfAR, TtSO, SCHR, DR, NDtRI, RBR, ICUR, NumDays = [child.value for child in spread_factors.children]
